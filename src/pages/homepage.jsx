...
export default function Home() {
    ...
    useEffect(() => {
        ...
        // Force refresh if cache doesn't have the new curated event types or if it's from a different day
        const hasCuratedEvents = dashboardData?.events?.some(e => ['iss', 'planets', 'meteors'].includes(e.type));
        const isDataFresh = dashboardData?.cachedDate === today;

        if (dashboardData && dashboardData.apod && hasCuratedEvents && isDataFresh) return;

        const fetchDashboard = async (lat, lng) => {
            try {
                const token = await getToken();
                if (!token) {
                    console.error("No Clerk token found. User might be logged out or Clerk failed to load.");
                    setLoading(false);
                    return;
                }
                ...
                // Add current date to data for caching logic
    ...
    if (loading) {
        return <div className="text-white text-center mt-40 font-orbitron">Loading Cosmic Dashboard...</div>;
    }

    if (!dashboardData) {
        return <div className="text-white text-center mt-40 font-orbitron">No cosmic data available</div>;
    }
    ...
    return (
        <div className="text-white font-orbitron min-h-screen bg-black">
            ...
            <section ref={eventsRef} className="px-6 py-10 max-w-5xl mx-auto">
                <h2 className="text-3xl font-bold mb-10 text-center tracking-tight">Cosmic Events</h2>

                {filteredEvents.length === 0 ? (
                    <p className="text-center text-white/40 italic py-10">No events found in this category.</p>
                ) : (
