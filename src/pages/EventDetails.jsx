    ...
    const fetchEvent = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/event-details/${id}?lat=${lat}&lng=${lng}`,
          {
            headers: {
    ...
    <div className="min-h-screen bg-black text-white font-orbitron relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 bg-[#050505]">
        <img
          src={
            event.image ||
            "https://images.unsplash.com/photo-1464802686167-b939a6910659?auto=format&fit=crop&q=80&w=2000"
          }
          alt=""
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="flex flex-wrap gap-4 mb-6">
                ...
                {event.id?.includes("asteroid") ||
                event.id?.includes("solar") ||
                event.id?.includes("iss") ? (
            ...
            <section className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-3xl">
              ...
              <ul className="space-y-6">
                {event.tips?.map((tip, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-5 text-white/80 group"
                  >
            ...
            <section className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-3xl">
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                {event.dark_spots?.map((spot, i) => (
                  <div
                    key={i}
                    className="p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-blue-500/10 hover:border-blue-500/50 transition-all group cursor-default"
                  >
                    <h4 className="font-bold text-xl text-white group-hover:text-blue-300 mb-2">
                      {spot.name}
                    </h4>
          ...
          <div className="space-y-8">
            <div className="p-8 bg-blue-500/10 backdrop-blur-xl border border-blue-500/30 rounded-[2.5rem] sticky top-32">
              ...
              <div className="space-y-10">
                <div>
                  <label className="text-[10px] text-white/30 uppercase tracking-[0.2em] block mb-2 font-bold">
                    Visibility Grade
                  </label>
                ...
                <div>
                  ...
                  <span className="text-3xl font-black text-white">
                    {event.weather?.condition || "Optimal"}
                  </span>
                </div>
                <div>
                  <label className="text-[10px] text-white/30 uppercase tracking-[0.2em] block mb-2 font-bold">
              ...
              <div className="mt-12 pt-8 border-t border-white/10">
                <p className="text-[10px] text-white/20 uppercase leading-relaxed tracking-wider">
                  Observation data is synced with real-time orbital mechanics
